...
function doPost(e) {
  ...
  try {
    ...
    var trailingHeaders = [
      "Ordered Items Summary (சுருக்கம்)",
      "Total Weight / Units Ordered (மொத்த Kg)",
    ...
    if (existingHeaders.length === 0) {
      ...
      itemKeys.forEach(function(key) {
        if (existingHeaders.indexOf(key) === -1) {
          var insertIdx = existingHeaders.indexOf("Ordered Items Summary (சுருக்கம்)");
    ...
    var newRow = existingHeaders.map(function(header) {
      switch(header) {
        ...
        case "Ordered Items Summary (சுருக்கம்)":
          return data.itemsSummary || "";
        case "Total Weight / Units Ordered (மொத்த Kg)":
